...
#' @param stanfit Logical; if `TRUE`, return both the `CmdStanMCMC` fit and the
...
#'   means. If `stanfit = TRUE`, a list with `stanfit` (a `CmdStanMCMC` object)
#'   and `results` (the vector).
#'
#' @examples
#' library(mpoly)
#' \dontrun{
...
variety_solve <- function(polylist,
  ...
  n_eqns <- if (is.mpoly(polylist)) 1L else length(polylist)
  n_vars <- length(vars)
  if (n_eqns != n_vars) {
  ...
  if (length(missing_vars) > 0L) {
    stop(
      "`vars` must name columns present in the draws; missing: ",
  ...
  n <- min(n, nrow(df))
  df <- df[(nrow(df) - n + 1):nrow(df), vars, drop = FALSE]
  row.names(df) <- NULL
